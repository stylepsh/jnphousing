...
async function fetchTodos(): Promise<{ open: TodoRow[]; done: TodoRow[]; tableMissing: boolean }> {
  if (!isSupabaseConfigured()) return { open: [], done: [], tableMissing: false };
  const supabase = await createClient();
  const [openRes, doneRes] = await Promise.all([
    supabase.from("team_todos").select("*").eq("status", "todo")
...
export default async function TodosPage() {
  const { open, done, tableMissing } = await fetchTodos();
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl">
      ...
      {tableMissing ? (
        ...
        <TodosClient open={open} done={done} todayIso={todayIso} />
      )}
    </div>
  );
}
