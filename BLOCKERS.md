...
### [B-109] PII 암호화 키 운영 설정 — 확인 필요(파괴 주의)
- **상황**: 계좌·주민·사업자번호 AES-256-GCM 암호화 코드 연결 완료. `crypto-pii.ts` 는 fail-closed —
  운영에서 키가 없거나 형식이 틀리면 평문 저장이 아니라 **저장 자체가 실패**한다.
- **현재 상태(2026-09-08 확인)**: Vercel(jnphousing) 에 `PII_ENCRYPTION_KEY` 가 Production·Preview 로
  등록돼 있다(100일 전). 단, Sensitive 타입이라 `vercel env pull` 로는 값이 빈 문자열로 내려와
  **값이 올바른 64자 hex 인지 CLI 로는 확인할 수 없다**(다른 키들도 전부 동일하게 빈 값으로 내려온다).
- **⚠️ 절대 하지 말 것**: "값이 비어 보인다"는 이유로 키를 지우고 새로 넣는 것. 정상 키였다면
  기존 암호문을 영구히 복호화할 수 없게 된다.
- **확인 방법**: 운영 화면에서 임대인 계좌번호를 한 번 저장해 본다. 성공하면 키 정상,
  "PII 암호화 키가 없어 저장할 수 없습니다" 가 뜨면 그때 키를 새로 설정한다.
